...
if(@$url[2]=='add'){
	$tpl['meta'] = '<title>'.$l['article_add_new'].' - OXCAKMAK</title><style>.ck { height: 400px; }</style>';
    $tpl['body'] = '
    <div class="row align-items-center justify-content-center">
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title mb-3">'.$l['article_add_new'].'&nbsp;<a class="btn btn-danger btn-sm waves-effect waves-light ms-2" href="'.$config['admin'].'articles">'.$l['comeback'].'</a></h4>
                    <div class="row">
                        ...
                        <div class="form-floating col-md-12 mb-3">
                            <input type="text" class="form-control thumbnail" id="flinthumbnail" placeholder="'.$l['picture'].'">
                            <label for="flinthumbnail">'.$l['picture'].'</label>
                        </div>
                        <div class="form-floating col-md-12 mb-3">
    ...
    $(".sendBtn").click(function(e){
        ...
        $.ajax({
            type: "POST",
            url: "'.$config['ajax'].'",
            data: {title: $(".title").val(), thumbnail: $(".thumbnail").val(), content: myEditor.getData(), description: $(".description").val(), action:"addArticle"},
            dataType: "json",
            success: function(r){
    ...
    $id = (($url[3])?$url[3]:'');
    $row = dbGetOne("id", $id, "article");
    if(!$row['id']){ header('location:'.$config['admin'].'articles'); }
	$tpl['meta'] = '<title>'.$l['article_edit'].' - OXCAKMAK</title><style>.ck { height: 400px; }</style>';
    $tpl['body'] = '
    <div class="row align-items-center justify-content-center">
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title mb-3">'.$l['article_edit'].'&nbsp;<a class="btn btn-danger btn-sm waves-effect waves-light ms-2" href="'.$config['admin'].'articles">'.$l['comeback'].'</a></h4>
                    <div class="row">
                        <div class="form-floating col-md-12 mb-3">
                            <input type="text" class="form-control title" id="flintitle" placeholder="'.$l['title'].'">
                            <label for="flintitle">'.$l['title'].'</label>
    ...
    $(".updateBtn").click(function(e){
        ...
        $.ajax({
            type: "POST",
            url: "'.$config['ajax'].'",
            data: {id: '.$row['id'].', title: $(".title").val(), thumbnail: $(".thumbnail").val(), content: myEditor.getData(), description: $(".description").val(), action:"editArticle"},
            dataType: "json",
            success: function(r){
                alert(r.title, r.content, r.type);
                /* If status not 1 then remove button disable status and if status 1 then clear all inputs */
	...
	<title>'.$l['articles'].' - OXCAKMAK</title>';
	$tpl['body'] = '
    <div class="row align-items-center justify-content-center">
        <div class="col-xl-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title mb-3">'.$l['articles'].'&nbsp;<a class="btn btn-primary btn-sm waves-effect waves-light ms-2" href="'.$config['admin'].'articles/add">'.$l['article_add_new'].'</a></h4>
    ...
    function remove(i, p){
        ...
        Swal.fire({
            title: "'.$l['question_delete_article'].'".replace("_ARTICLE_", p),
            icon: "question",
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "'.$config['ajax'].'",
                    data: {id: i, action:"deleteArticle" },
                    dataType: "json",
                    success: function(r){
                        alert(r.title, r.content, r.type);
                        /* If status not 1 then remove button disable status and if status 1 then clear all inputs */
